...
public static void main() {
    ...
    String msg = "00";
    String ad = "00010203";
    String ciphertext = "14F7291EEA8AC65556D6EAFE14FCA74732";

    int[] msgInt = byteToIntegerArray(stringToByteArray(msg));
    int[] keyInt = byteToIntegerArray(stringToByteArray(key));
    int[] adInt = byteToIntegerArray(stringToByteArray(ad));
    int[] nonceInt = byteToIntegerArray(stringToByteArray(nonce));

    System.out.println("CT is: " + crypto_aead_encrypt(msgInt, adInt
    , nonceInt, keyInt));

}

public static int[] byteToIntegerArray(byte[] msgBytes) {
    int shiftBits;
    int byteNum = 0;
    int[] msgInt = new int[msgBytes.length/4]; //you might have to hard code the array length

    //Convert array of source bytes (srcByte) into array of integers (dstInt)
    for (int intNum = 0; intNum < msgBytes.length/4; ++intNum) {  //for the four integers
        msgInt[intNum] = 0;                                      //Start with the integer = 0

        for(shiftBits = 0; shiftBits < 32; shiftBits += 8) {     //Add in each data byte, lowest first
            msgInt[intNum] |= (msgBytes[byteNum++] & 0xFF) << shiftBits;
        }
    }
    return msgInt;
}
...
public static byte[] stringToByteArray(String msg) {
    byte[] msgBytes = new byte[msg.length() / 2];
        for (int i = 0; i < msgBytes.length; i++) {
            msgBytes[i] = (byte) Integer.parseInt(msg.substring(i * 2, i * 2 + 2), 16);
        }
    return msgBytes;
}
